...
const NAV: Array<{ href: string; label: string; group?: string }> = [
  ...
  { href: '/admin/verify',         label: 'Verify queue (CCIM)', group: 'directory' },
  { href: '/admin/verify-badges',  label: 'Credential badges',   group: 'directory' },
  ...
  { href: '/admin/applications-report', label: 'Applications report', group: 'directory' },
    ...
    return (
      <div className="min-h-screen flex items-center justify-center p-8">
        <div className="max-w-md text-center space-y-3">
          ...
          <Link href="/" className="inline-block mt-4 text-kerala-700 hover:underline">← back to site</Link>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="min-h-screen flex bg-gray-50">
      <aside className="w-60 border-r bg-white py-6 px-4 sticky top-0 self-start h-screen overflow-y-auto">
        <Link href="/admin" className="block mb-6 text-lg font-bold text-kerala-800">
          AyurConnect <span className="text-gray-500 font-normal">/ admin</span>
        </Link>
        <nav className="space-y-5 text-sm">
          {groups._top?.map((it) => (
            <Link key={it.href} href={it.href} className="block px-2 py-1.5 rounded hover:bg-kerala-50 text-gray-800">
              {it.label}
            </Link>
          ))}
        ...
        <div className="mt-8 pt-4 border-t text-xs text-gray-500 space-y-1">
          ...
          <div>role: <span className="text-kerala-700 font-mono">{sess.user.role}</span></div>
          <Link href="/" className="block mt-2 text-kerala-700 hover:underline">← public site</Link>
        </div>
      </aside>
