// ISO 3166-1 alpha-2 country codes with E.164 dial codes and Unicode flag emoji.
// Kept hand-curated and lightweight (~250 rows, ~15KB). No images — flag emoji
// works on every modern OS.
//
// Source: ITU-T E.164 country codes + ISO 3166-1. Verified against the official
// numbering plan as of 2025-11. A handful of microstates / disputed territories
// are intentionally omitted.

export type Country = {
  code: string  // ISO 3166-1 alpha-2 (uppercase)
  name: string  // English short name
  dial: string  // E.164 dial code WITHOUT plus, e.g. '91', '44', '1'
  flag: string  // Unicode flag emoji
}

// Kerala diaspora hotspots float to the top of the picker.
// All countries listed here are in COUNTRIES below — these are just hints.
export const PRIORITY_CODES = ['IN', 'AE', 'SA', 'QA', 'KW', 'OM', 'BH', 'US', 'GB', 'CA', 'AU', 'SG', 'MY', 'NZ', 'IE'] as const
...
export const COUNTRIES: Country[] = [
  ...
  ['CZ', 'Czech Republic', '420'], ['CD', 'DR Congo', '243'], ['DK', 'Denmark', '45'], ['DJ', 'Djibouti', '253'],
  ...
  ['SZ', 'Eswatini', '268'], ['ET', 'Ethiopia', '251'], ['FK', 'Falkland Islands', '500'], ['FO', 'Faroe Islands', '298'],
  ...
  ['GL', 'Greenland', '299'], ['GD', 'Grenada', '1473'], ['GP', 'Guadeloupe', '590'], ['GU', 'Guam', '1671'],
  ['GT', 'Guatemala', '502'], ['GG', 'Guernsey', '44'], ['GN', 'Guinea', '224'], ['GW', 'Guinea-Bissau', '245'],
  ['GY', 'Guyana', '592'], ['HT', 'Haiti', '509'], ['HN', 'Honduras', '504'], ['HK', 'Hong Kong', '852'],
  ['HU', 'Hungary', '36'], ['IS', 'Iceland', '354'], ['IN', 'India', '91'], ['ID', 'Indonesia', '62'],
  ['IR', 'Iran', '98'], ['IQ', 'Iraq', '964'], ['IE', 'Ireland', '353'], ['IM', 'Isle of Man', '44'],
  ['IL', 'Israel', '972'], ['IT', 'Italy', '39'], ['CI', 'Ivory Coast', '225'], ['JM', 'Jamaica', '1876'],
  ['JP', 'Japan', '81'], ['JE', 'Jersey', '44'], ['JO', 'Jordan', '962'], ['KZ', 'Kazakhstan', '7'],
  ['KE', 'Kenya', '254'], ['KI', 'Kiribati', '686'], ['XK', 'Kosovo', '383'], ['KW', 'Kuwait', '965'],
  ...
  ['LT', 'Lithuania', '370'], ['LU', 'Luxembourg', '352'], ['MO', 'Macao', '853'], ['MG', 'Madagascar', '261'],
  ['MW', 'Malawi', '265'], ['MY', 'Malaysia', '60'], ['MV', 'Maldives', '960'], ['ML', 'Mali', '223'],
  ['MT', 'Malta', '356'], ['MH', 'Marshall Islands', '692'], ['MQ', 'Martinique', '596'], ['MR', 'Mauritania', '222'],
  ['MU', 'Mauritius', '230'], ['YT', 'Mayotte', '262'], ['MX', 'Mexico', '52'], ['FM', 'Micronesia', '691'],
  ['MD', 'Moldova', '373'], ['MC', 'Monaco', '377'], ['MN', 'Mongolia', '976'], ['ME', 'Montenegro', '382'],
  ['MS', 'Montserrat', '1664'], ['MA', 'Morocco', '212'], ['MZ', 'Mozambique', '258'], ['MM', 'Myanmar', '95'],
  ['NA', 'Namibia', '264'], ['NR', 'Nauru', '674'], ['NP', 'Nepal', '977'], ['NL', 'Netherlands', '31'],
  ['NC', 'New Caledonia', '687'], ['NZ', 'New Zealand', '64'], ['NI', 'Nicaragua', '505'], ['NE', 'Niger', '227'],
  ['NG', 'Nigeria', '234'], ['NU', 'Niue', '683'], ['KP', 'North Korea', '850'], ['MK', 'North Macedonia', '389'],
  ['MP', 'Northern Mariana Islands', '1670'], ['NO', 'Norway', '47'], ['OM', 'Oman', '968'], ['PK', 'Pakistan', '92'],
  ['PW', 'Palau', '680'], ['PS', 'Palestine', '970'], ['PA', 'Panama', '507'], ['PG', 'Papua New Guinea', '675'],
  ['PY', 'Paraguay', '595'], ['PE', 'Peru', '51'], ['PH', 'Philippines', '63'], ['PN', 'Pitcairn', '64'],
  ...
  ['RW', 'Rwanda', '250'], ['BL', 'Saint Barthélemy', '590'], ['SH', 'Saint Helena', '290'], ['KN', 'Saint Kitts and Nevis', '1869'],
  ...
  ['ST', 'São Tomé and Príncipe', '239'], ['SA', 'Saudi Arabia', '966'], ['SN', 'Senegal', '221'],
  ['RS', 'Serbia', '381'], ['SC', 'Seychelles', '248'], ['SL', 'Sierra Leone', '232'], ['SG', 'Singapore', '65'],
  ['SX', 'Sint Maarten', '1721'], ['SK', 'Slovakia', '421'], ['SI', 'Slovenia', '386'], ['SB', 'Solomon Islands', '677'],
  ...
  ['TN', 'Tunisia', '216'], ['TR', 'Turkey', '90'], ['TM', 'Turkmenistan', '993'], ['TC', 'Turks and Caicos Islands', '1649'],
  ...
  ['UZ', 'Uzbekistan', '998'], ['VU', 'Vanuatu', '678'], ['VA', 'Vatican City', '39'], ['VE', 'Venezuela', '58'],
  ['VN', 'Vietnam', '84'], ['WF', 'Wallis and Futuna', '681'], ['EH', 'Western Sahara', '212'], ['YE', 'Yemen', '967'],
  ['ZM', 'Zambia', '260'], ['ZW', 'Zimbabwe', '263'],
].map(([code, name, dial]) => ({ code, name, dial, flag: flagOf(code) }))

export const COUNTRY_BY_CODE: Record<string, Country> =
  Object.fromEntries(COUNTRIES.map((c) => [c.code, c]))

export function findCountryByDial(dial: string): Country | null {
  // Greedy match — '+1' has many matches, but we want United States as default.
  // Sort priority codes first, then by length descending so '1284' beats '1'.
  const candidates = COUNTRIES
    .filter((c) => c.dial === dial)
    .sort((a, b) => {
      const ap = (PRIORITY_CODES as readonly string[]).indexOf(a.code)
      const bp = (PRIORITY_CODES as readonly string[]).indexOf(b.code)
